...
from logic.workerThread import WorkerThread
...
class ldaLogic(WorkerThread):
    ...
    def read_in_streams(self):
        ...
        self.lda_config.set_feature_description(self.main_features + self.context_features)
    ...
    def make_context_words(self, stream, start, end, stats, hour, day):
        words = []
        for feature in self.context_features:
            if stats[feature]['max'] == stats[feature]['min']:
                words.extend(self.make_special_words(stats[feature]['max'], start, end, hour, day))
                continue
            ...
            while word_start < end:
                ...
                words.append(self.make_word(chunk, alphabet, distribution, x_grid, hour, day))
                word_start += self.lda_config.word_duration
        return words

    def make_main_words(self, stream, start, end, stats, sensor_id, hour, day):
        words = []
        for feature in self.main_features:
            if stats[sensor_id][feature]['max'] == stats[sensor_id][feature]['min']:
                words.extend(self.make_special_words(stats[sensor_id][feature]['max'], start, end, hour, day))
                continue
            distribution, x_grid = stream.get_pdf_of_time_window(feature, start, end)
            word_start = start
            alphabet = self.lda_config.alphabets[feature]
    ...
    def make_special_words(self, value, start, end, hour, day):
        words = []
        while start < end:
            words.append("a"+hour+day)
            start += self.lda_config.word_duration
        return words

    def make_word(self, chunk, alphabet, distribution, x_grid, hour, day):
    ...
    def doWork(self):
        current_date = datetime.strptime("2014-08-02T08", self.time_format)
        ...
        while current_date < end_date:
            ...
            weather_stats = {
                ...
                for feature in self.context_features}
            weather_words = self.make_context_words(self.context_stream, current_date, end_of_document_window_date,
            ...
            for sensor_id, stream in self.main_streams.items():
                ...
                try:
                    traffic_stats[sensor_id] = {feature: stream.get_statistics(feature, current_date,
                                                ...
                                                for feature in self.main_features}
                    traffic_words = self.make_main_words(stream, current_date, end_of_document_window_date,
            ...
            if first_run:
                ...
                write_new_topics2file(current_date, new_doc_ldas, id2token)
                #
